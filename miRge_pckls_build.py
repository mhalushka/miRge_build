...
def main(arg = sys.argv):
	if len(arg) != 4:
		print >> sys.stderr, 'build_lib.py species PathOfGRCh38_genome_repeats.GTF PathOfHuman_genome.fa'
		sys.exit(1)
	else:
		species = arg[1]
		...
		cPickle.dump(repEleChrCoordinateDic,f)
		f.close()
		time2 = time.time()
		...
		for record in SeqIO.parse(PathOfHuman_genome, "fasta"):
			chr, seq = record.id, record.seq.tostring()
			chrSeqDic.update({chr:seq})
		f = open(os.path.join(os.path.split(PathOfHuman_genome)[0], species+'_genome.pckl'), "wb")
		cPickle.dump(chrSeqDic,f)
		f.close()
		time6 = time.time()
		print "Transforming %s_genome.fa takes: %.1fs"%(species, time6-time5)
